<?php
    ...
    class XmlParser {
        ...
        public static function parse ($file) {
            ...
            if (!$text) {
                throw new Exception('Resource "'.htmlspecialchars($file).'" is unavailable for reading.');
            }
            ...
            if (!$dom) {
                throw new Exception('Resource "'.htmlspecialchars($file).'" isn\'t XML document.');
            }
			
			$request_params = $dom->getElementsByTagName('request')->item(0);
			if (!$request_params) {
				throw new Exception('Request doesn\'t have correct parameters.');
			}
			$res['request_params']['token'] = $request_params->getAttribute('token');
			$res['request_params']['isPublic'] = $request_params->getAttribute('isPublic');
			$res['request_params']['channel'] = $request_params->getAttribute('channel');
			...
			$collection = $dom->getElementsByTagNameNS('*', 'GeoObjectCollection')->item(0);
			
			if ($collection) {
				$res['collection'] = self::mapCollection($collection, array());
            }
			/* TODO: Need to be redone! 
            foreach ($dom->documentElement->childNodes as $elem) {
