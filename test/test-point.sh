...
./update-point.sh ${NAME} ${PNAME} description 45 2>/dev/null >/dev/null
psql -U geo2tag geo2tag -c "select count(*) from tag where label='${PNAME}' and description='45'" | head -n 3 | tail -n 1 | grep 1 && echo "Test passed" || echo "Test failed"
