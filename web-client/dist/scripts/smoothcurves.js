...
MonotonicCubicSpline = (function() {
  function MonotonicCubicSpline(x, y) {
    var alpha, beta, delta, dist, i, j, k, l, len, len1, m, n, o, p, q, ref, ref1, ref2, ref3, tau, to_fix;
    n = x.length;
    delta = [];
    m = [];
    alpha = [];
    ...
    for (l = 0, len = to_fix.length; l < len; l++) {
      i = to_fix[l];
      m[i] = m[i + 1] = 0;
    }
    for (i = o = 0, ref2 = n - 1; 0 <= ref2 ? o < ref2 : o > ref2; i = 0 <= ref2 ? ++o : --o) {
      alpha[i] = m[i] / delta[i];
    ...
    to_fix = [];
    for (i = p = 0, ref3 = n - 1; 0 <= ref3 ? p < ref3 : p > ref3; i = 0 <= ref3 ? ++p : --p) {
      if (dist[i] > 9) {
        to_fix.push(i);
      }
    }
    for (q = 0, len1 = to_fix.length; q < len1; q++) {
      i = to_fix[q];
  ...
  MonotonicCubicSpline.prototype.interpolate = function(x) {
    var h, h00, h01, h10, h11, i, j, ref, t, t2, t3, y;
